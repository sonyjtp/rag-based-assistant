...
class TestSlidingWindowMemoryInitialization:
    ...
    def test_init_with_defaults(self):
        ...
        memory = SlidingWindowMemory(llm=mock_llm)
    ...
    def test_init_with_custom_window_size(self):
        ...
        memory = SlidingWindowMemory(llm=mock_llm, window_size=10)
    ...
    def test_init_with_custom_memory_key(self):
        ...
        memory = SlidingWindowMemory(llm=mock_llm, memory_key="conversation")
    ...
    def test_messages_is_deque_with_maxlen(self):
        ...
        memory = SlidingWindowMemory(llm=mock_llm, window_size=window_size)
...
class TestSlidingWindowMemorySaveContext:
    ...
    def test_save_single_message(self):
        ...
        mock_llm = MagicMock()
        memory = SlidingWindowMemory(llm=mock_llm, window_size=5)
    ...
    def test_window_full_triggers_summarization(self):
        ...
        memory = SlidingWindowMemory(llm=mock_llm, window_size=3)

        # Fill the window
        for i in range(3):
            memory.save_context(
                inputs={"input": f"Q{i}"},
                outputs={"output": f"A{i}"}
            )
    ...
    def test_save_context_extracts_output_text(self):
        ...
        memory.save_context(
            inputs={"input": "What is the capital?"},
...
class TestSlidingWindowMemorySummarization:
    ...
    def test_summarize_window_calls_llm(self):
        ...
        mock_llm = MagicMock()
        mock_llm.invoke.return_value = MagicMock(content="Summary")
        memory = SlidingWindowMemory(llm=mock_llm, window_size=2)
...
class TestSlidingWindowMemoryLoadMemoryVariables:
    ...
    def test_load_memory_variables_empty(self):
        ...
        mock_llm = MagicMock()
        memory = SlidingWindowMemory(llm=mock_llm, memory_key="chat_history")
...
class TestSlidingWindowMemoryEdgeCases:
    ...
    def test_special_characters_in_message(self):
        ...
        special_text = "!@#$%^&*()_+-=[]{}|;:',.<>?/`~\n\t"
        memory.save_context(
            inputs={"input": special_text},
            outputs={"output": special_text}
        )
    ...
    def test_unicode_characters(self):
        ...
        memory.save_context(
            inputs={"input": unicode_text},
            outputs={"output": unicode_text}
        )
    ...
    def test_window_size_one(self):
        ...
        mock_llm = MagicMock()
        mock_llm.invoke.return_value = MagicMock(content="Summary")
        memory = SlidingWindowMemory(llm=mock_llm, window_size=1)
